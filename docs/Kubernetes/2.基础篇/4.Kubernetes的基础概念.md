...
![](../../assets/static/image-20230201160710333.png)
...
![](../../assets/static/image-20230201160915690.png)

图4.2Pod图


由图可知，Pod可简单地理解为一组、一个或多个容器，每个Pod还包含一个Pause容器，Pause容器是Pod的父容器，它主要负责僵尸进程的回收管理，
同时**通过Pause容器可以使同一个Pod里面的不同容器共享存储、网络、PID、IPC等，容器之间可以使用localhost:port相互访问，可以使用Volume等实现数据共享。**
