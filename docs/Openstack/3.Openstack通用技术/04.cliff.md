...
class PrettyPrinter:
    ...
    def print(cls, data):
        ...
        if isinstance(data, list):
            cls._print_list(data)
        elif isinstance(data, dict):
            cls._print_dict(data)
        else:
            print(data)

    @classmethod
    def _print_dict(cls, data):
        """处理字典类型的打印"""
        if not data:
            print("{}")
            return
        ...
        # 计算列宽
        key_width = max(len(str(k)) for k in keys) + 2
        val_width = max(len(str(v)) for v in values) + 2

        # 打印表头
        print(f"+{'-' * (key_width + val_width + 1)}+")
        print(f"| {'Key'.ljust(key_width)}| {'Value'.ljust(val_width)}|")
        print(f"+{'-' * (key_width + val_width + 1)}+")

        # 打印数据行
        for k, v in data.items():
            row = f"| {str(k).ljust(key_width)}| {str(v).ljust(val_width)}|"
            print(row)

        print(f"+{'-' * (key_width + val_width + 1)}+")

    @classmethod
    def _print_list(cls, data):
        """处理列表类型的打印"""
        if not data:
            print("[]")
            return
        ...
        if all(isinstance(item, dict) for item in data):
            # 收集所有可能的键
            all_keys = list(set(key for item in data for key in item.keys()))

            # 计算列宽
            col_widths = {
                key: max(len(str(key)),
                        max(len(str(item.get(key, ''))) for item in data)) + 2
                for key in all_keys
            }

            # 打印表头
            header = "| " + " | ".join(
                str(key).ljust(col_widths[key]) for key in all_keys
            ) + " |"
            separator = "+" + "+".join(
            ...
            for item in data:
                row = "| " + " | ".join(
                    str(item.get(key, '')).ljust(col_widths[key])
                    for key in all_keys
                ) + " |"
                print(row)
            print(separator)
...
if __name__ == "__main__":
    ...
    PrettyPrinter.print(data_dict)

    print("\nList of Dictionaries Output:")
    PrettyPrinter.print(data_list)
```
