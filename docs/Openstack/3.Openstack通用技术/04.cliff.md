...
def print_list(objs, fields, formatters={}, order_by=None):
    ...
    # 打印表头
    header = '|'.join(f"{field:<{width}}" for field, width in zip(fields, column_widths))
        ...
        for i, field in enumerate(fields):
            if field in formatters:
                ...
                if isinstance(o, dict) and field in o:
                    ...
                    value = str(getattr(o, field_name, ''))
            row.append(f"{value:<{column_widths[i]}}")
        row_str = '|'.join(row)
        print(f"|{row_str}|")
    print(f"+{separator}+")
...
def print_dict(d, property="Property"):
    ...
    for key, value in d.items():
        row = f"{str(key):<{key_width}}|{str(value):<{value_width}}"
        print(f"|{row}|")
    print(f"+{separator}+")
...
if __name__ == '__main__':
    data = [
        ...
        {'name': 'Jim', 'age': 45, 'city': 'Paris'},
    ]
    print_list(data, ['name', 'age', 'city'])
