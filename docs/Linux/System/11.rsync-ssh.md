...
# 免密同步远程目录到本地目录放置后台操作
nohup sshpass -p "xxxx"  rsync -avzP -e "ssh -p 22" root@192.168.1.60:/data/nfs/global/repositories /data/repositories > rsync_$(date +%F).log 2>&1 &
# 免密同步本地目录到远程并放后台处理
