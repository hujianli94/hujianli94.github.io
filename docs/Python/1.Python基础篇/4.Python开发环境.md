...
* \_: 打印前输出结果
* %run：命令当做Python程序来运行，输入%run 路径+文件名称即可，运行完毕会导入文件的变量、函数、方法到当前环境中
* %run -d: 交互式调试器
* ! shell_command: 执行shell命令
* %load：加载代码%load命令用于将脚本代码加载到当前cell。
* %save：保存cell %save path.py n1 n2..命令用于将指定cell代码保存到指定的py文件中。
* %debug: 交互式调试器 %debug命令支持从最新的异常跟踪的底部进入交互式调试器。在ipdb调试模式下能访问所有的本地变量和整个栈回溯。使用u和d向上和向下访问栈，使用q退出调试器。在调试器中输入?可以查看所有的可用命令列表。
...
```json
  ...
  "Ansible Playbook with Vars": {
    ...
    "description": "Ansible Playbook with Vars"
  },
  "Ansible Task": {
    "prefix": "at",
    "body": [
  ...
  "Ansible Task with Command": {
    "prefix": "atc",
    "body": [
      "- name: $1",
      "  command: $2"
    ],
    "description": "Ansible Task with Command"
  },
  "Ansible Task with Shell": {
    "prefix": "ats",
    "body": [
      "- name: $1",
      "  shell: $2"
    ],
    "description": "Ansible Task with Shell"
  },
  "Ansible Task with Copy": {
    "prefix": "atcp",
    "body": [
      "- name: $1",
      "  copy:",
    ...
    "description": "Ansible Task with Copy"
  },
  "Ansible Task with Template": {
    "prefix": "att",
    "body": [
      "- name: $1",
      "  template:",
    ...
    "description": "Ansible Task with Template"
  },
}
```
