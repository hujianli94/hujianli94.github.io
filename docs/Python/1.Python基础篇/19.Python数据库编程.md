...
# 连接到数据库
db = records.Database('sqlite:///mydatabase.db')

# 创建表格（如果表格不存在）
db.query('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, age INTEGER)')
...
# 查询指定条件的记录
age_threshold = 30
result = db.query('SELECT * FROM users WHERE age > :age_threshold', age_threshold=age_threshold)
for row in result:
    print(row.id, row.name, row.age)
