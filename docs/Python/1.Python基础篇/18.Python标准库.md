...
def subp(args, data=None, rcs=None, env=None, capture=True, shell=False,
    ...
    """
    该函数的功能：
    1.通过subprocess.Popen执行外部命令或脚本

    2.输入与输出处理：
      - 可以向子进程发送输入数据data。
      - 可以捕获子进程的输出数据stdout和stderr。

    3.错误处理：
      - 如果返回代码不在预期的返回代码列表rcs中，则抛出ProcessExecutionError异常。

    4.环境变量管理：
      - 可以更新环境变量update_env。
    5. 编码处理：根据decode参数决定是否解码输出数据。

    6.特殊功能：
      - target参数支持chroot执行环境，但此功能在cloud-init中暂不支持。
    """
