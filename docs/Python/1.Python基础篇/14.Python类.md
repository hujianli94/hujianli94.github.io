...
5. 一般创建类会带有特定的初始状态
   使用**\_\_init\_\_**作为初始化方法，可以用来设置对象属性，并给予初始值，可以是参数或者固定值：
...
**\_\_init\_\_**函数定义到类的开头，self.name 变量是一个实例属性，只能在类方法中使用，引用时也要这样 self.name。
...
在单下划线基础上又加了一个下划线 `self.\_\_name = name`
...
（2）类方法在重新定义类的时候无须修改构造方法\_\_init\_\_()，将需要重构的代码以类方法的形式表示即可。
...
这里面\_\_init\_\_子类是重复定义了父类的，如果大量调用子类，这段也会多消耗一些资源。
...
当我们调用装饰器的时候，它会执行定义的\_\_call\_\_魔法方法，所以也就完成了一个装饰器的定义和调用。

\_\_call\_\_方法: :可以让类中的方法像函数一样调用。
