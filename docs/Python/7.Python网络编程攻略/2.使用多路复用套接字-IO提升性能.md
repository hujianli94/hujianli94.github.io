...
在 Python 中，类的构造方法叫作 `__init__()` 。按照惯例，要把 self 作为参数传入 `__init__()` 方法，以便指定具体实例的属性。
...
```sh
# (py38) root@hujianli722# python 2_1_forking_mixin_socket_server.py
# 2024-11-13 20:54:09,136 - INFO - Server started on 127.0.0.1:39995
# 2024-11-13 20:54:09,139 - INFO - Server received: b'Hello echo server!', sent back: Echo from server: b'Hello echo server!'
# 2024-11-13 20:54:09,139 - INFO - Received from server: Echo from server: b'Hello echo server!'
# 2024-11-13 20:54:09,140 - INFO - Client socket closed.
# 2024-11-13 20:54:09,140 - INFO - Server received: b'Hello echo server!', sent back: Echo from server: b'Hello echo server!'
# 2024-11-13 20:54:09,140 - INFO - Received from server: Echo from server: b'Hello echo server!'
# 2024-11-13 20:54:09,140 - INFO - Client socket closed.
```
...
```sh
# (py38) root@hujianli722# python 2_2_threading_mixin_socket_server.py
...
- 编写一个高效的聊天室服务器，处理几百或更多数量的客户端连接。
- 使用 select 模块提供的 select() 方法，让聊天室服务器和客户端所做的操作始终不会阻塞消息的发送和接收。
- 使用一个脚本就能启动客户端和服务器
- 在命令行中传入 --name=server ，脚本才启动聊天室服务器。
- 在命令行中传入 --name=client1 ，例如 client1 或 client2 ，则脚本会启动聊天室客户端。
- 聊天室服务器绑定的端口在命令行参数--port 中指定。

对大型应用程序而言，最好在不同的模块中编写服务器和客户端。
...
这个脚本要运行三次：一次用于启动聊天室服务器，两次用于启动两个聊天室客户端。

启动服务器时，在命令行中传入参数 –-name=server 和 --port=8800 。
...
```sh
# # server
# (py38) root@hujianli722# python 2_3_chat_server_with_select.py --name=server --port=8800
# Server listening to port: 8800...
# Chat server: got connection 4 from ('127.0.0.1', 46850)
# Chat server: got connection 5 from ('127.0.0.1', 51562)


# # client1
# (py38) root@hujianli722# python 2_3_chat_server_with_select.py --name=client1 --port=8800
...
# # client2
# # (py38) root@hujianli722# python 2_3_chat_server_with_select.py --name=client2 --port=8800
# #client1@127.0.0.1>>hello clinet2
# [client2@127.0.0.1]>hello client1
# [client2@127.0.0.1]>
```
...
```sh
# # Server
# (py38) root@hujianli722# python 2_4_simple_web_server_with_epoll.py --port 8800
...
# # clinet2
# (py38) root@hujianli722# localhost:8800
# Hello from Epoll Server!
```
