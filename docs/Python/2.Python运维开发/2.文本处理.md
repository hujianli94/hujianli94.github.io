...
有了用户IP以后，我们可以很方便地获取到网站的PV和UV（PV是网站的访问请求数，UV是网站的独立访客数）。
...
对于Counter来说，如果一个键不存在于计数器中，直接对这个键进行运算也不会报错，如上面这段代码中的“c['d']+=1”。
...
4. 去除缩进：使用textwrap.dedent()函数可以去除文本中的缩进。
...
例如，我们使用Linux下的seq命令产生1000万个整数保存到文件中。
...
虽然简单使用时match函数和startswith函数比较类似，但是，对于复杂的情况，match函数能够轻易解决，starswith则无能为力。
...
所以，re模块中的sub函数使用场景更加广泛，使用方式也更加灵活。

例如，下面的正则表达式模式，可以同时匹配"2.7.13"和"3.6.0"，并将它们都替换为"x.x.x"。

这个需求看似简单，如果使用Python字符串的replace函数，需要进行两次替换。

使用正则表达式的sub函数，只需要一次替换即可完成。如下所示：
...
在Python的re模块中，sub()和subn()函数都是用来替换字符串中的匹配项。它们的区别在于返回值和行为。

sub()函数返回替换后的新字符串，而subn()函数返回一个元组，其中第一个元素是替换后的新字符串，第二个元素是替换次数。
...
subn()函数的语法与sub()函数类似，只是返回值是一个元组，其中第一个元素是替换后的新字符串，第二个元素是替换次数。例如：
...
在index.html中，我们使用`{%extends"base.html"%}`继承base.html，继承以后，base.html中的所有内容都会在index.html中展现。
...
下面来看两个例子以巩固前面的知识。
如果读者在Flask中使用Jinja2，则几乎不需要关心Jinja2的使用。


因为Jinja2的开发者和Flask的开发者是同一个人，为了便于工程师在Flask中使用Jinja2，作者在Flask中很好地集成了Jinja2。

集成以后，想要在Flask中使用Jinja2，只需使用Flask包下的`render_template`函数访问模板即可。
...
可以看到，上面的代码将业务逻辑和表现逻辑混合在一起，显得比较混乱。这个时候，我们完全可以使用Jinja2模板解决这个问题。
