...
[将python包发布到PyPI和制作whl文件方式](https://www.jb51.net/article/177180.htm)
...
Setuptools 和较旧的 Distutils 都是用于发布Python包的工具包，让你能够使用Python轻松地编写安装脚本。
...
安装时不需要执行命令 build ，因为当你执行命令 install时，如果需要，命令build 会自动运行。
...
将hello.py作为包放置到`/usr/lib/python2.7/site-packages`中，可以直接当做包进行import导入
...
你还可使用它 来创建Windows安装程序、RPM包、egg文件、wheel文件等（wheel将最终取代egg）。

这里只介绍 如何创建.tar.gz文件，你应该能够根据文档轻松地创建其他格式的文件。 
...
现在，除目录build外，应该还有一个名为dist的目录。在这个目录中，有一个名为 hello-1.0.0.tar.gz 的文件。

你可将其分发给他人，而对方可将其解压缩，再使用脚本setup.py进行安装。
...
### 10.3 entry_points将Python模块转变为命令行工具
...
[一个MySQL简单操作方式](https://github.com/mouday/PureMySQL/blob/master/setup.py)
...
其中有个 console_scripts 的键，表示注册一个叫作pyHello的系统命令， 这个命令会调用 pyHello.hello 的 main 函数，
安装的时候由setuptools来帮助我们生成了/usr/local/bin/pyHello这个文件。 
...
使用 entry_points 的优点，就是可以让这些入口点能够被其他Python程序动态发现包所提供的功能，但是对应的代码的耦合度非常低。


[上传自己的pip模块](https://www.cnblogs.com/zhangyafei/p/14391536.html)
