...
class Category(db.Model):
    # ....
    newses = db.relationship("News", back_populates="category",cascade="delete")


class News(db.Model):
    # ....
    category = db.relationship("Category", back_populates="newses",cascade="delete")
```
...
merge是默认选项。在使用session.merge合并对象时，会将使用了db.relationship相关联的对象也进行merge操作。这个参数几乎很少用到，读者作为了解即可。
...
class Category(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(100))
    newses = db.relationship("News",back_populates="category",cascade="expunge")
