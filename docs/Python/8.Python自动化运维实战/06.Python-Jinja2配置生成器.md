...
python 的模板引擎，也是 Flask 的模板引擎，能根据模板字符串替换，有自己的语法规则。
...
jinja 的语法很灵活，也很丰富，下面就列举几个常用到的，详细见官网文档介绍。
...
可以在注释，for，if 或者变量表达式的开始或者结束放一个’-‘，可以移除块前或者后的空白。
...
程序文件 `jinja2_template_dockerfile.py`
...
FROM centos:7
LABEL maintainer="hujianli" name="demo" build-date="20250412"
