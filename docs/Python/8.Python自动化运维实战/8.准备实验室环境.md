...
CentOS 是一个以社区驱动的 Linux 操作系统，面向企业服务器，并以其与Red Hat Enterprise Linux（RHEL）的兼容性而闻名。
...
- CentOS下可以执行 `yum install kvm` 安装



客户机驱动：Linux客户机的驱动包含在内核中；[Windows客户机的驱动](http://www.linux-kvm.org/page/WindowsGuestDrivers/Download_Drivers)需要下载
...
说明：启动没有报错的话，主机上会生成 5900 端口(默认)，通过vnc客户端连接即可
