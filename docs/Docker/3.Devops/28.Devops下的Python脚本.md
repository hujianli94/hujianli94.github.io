...
"""
一行命令安装docker：
mkdir tmp_for_setup_docker && yum install -y git && git clone https://gitee.com/shihowcom/setup_docker.git tmp_for_setup_docker/setup_docker && python ./tmp_for_setup_docker/setup_docker/setup_docker.py
"""
import os
import shutil
import subprocess
...
DAEMON_JSON = '''{
    "registry-mirrors": ["https://roq16bcx.mirror.aliyuncs.com"],
...
def main():
    ...
    print("清理 tmp_for_setup_docker 开始")
    shutil.rmtree('./tmp_for_setup_docker', ignore_errors=True) 
    print("清理 tmp_for_setup_docker 完成")
...
def uninstall():
    ...
    :return:
    """
    os.system("rpm -e --nodeps `rpm -qa |grep mysql`")
    os.system("rpm -e --nodeps `rpm -qa |grep mariadb`")
...
def install():
    ...
    :return:
    """
    os.system("mkdir mysql")
    os.system("tar -xvf mysql-5.7.36-1.el7.x86_64.rpm-bundle.tar -C ./mysql")
    os.chdir("mysql")
    os.system("rpm -ivh ./mysql-community-common-5.7.36-1.el7.x86_64.rpm")
    os.system("rpm -ivh ./mysql-community-libs-5.7.36-1.el7.x86_64.rpm")
    os.system("rpm -ivh ./mysql-community-client-5.7.36-1.el7.x86_64.rpm")
    os.system("rpm -ivh ./mysql-community-server-5.7.36-1.el7.x86_64.rpm")
    os.system("rpm -ivh ./mysql-community-devel-5.7.36-1.el7.x86_64.rpm")
...
def detect():
    ...
    """
    time.sleep(5)
    return True if os.popen("systemctl status mysqld").read().find(
...
def get_default_password():
    ...
    :return:
    """
    line = os.popen("grep \"temporary password\" /var/log/mysqld.log").read().decode("UTF-8")
...
def prompt_success():
    ...
    """
    print """
    \033[5;32;40m 恭喜mysql_5.7.36安装成功！你root登录密码是: {0} \033[0m
...
## 多线程爬取图片的例子（centos6.8 x86_64下通过）¶
