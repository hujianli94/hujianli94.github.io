...
- make关键字的作用是创建于slice、map、channel等内置的数据结构
- new的作用是为类型申请一片内存空间，并返回这片内存的指针
...
类似go build，只是编译中间文件放在 `$GOPATH/pkg` 目录下，编译结果放在 `$GOPATH/bin` 目录下。


"go install" 命令的输出目录始终为`$GOPATH` 下的 bin 目录，无法使用-o附加参数进行自定义。`$GOPATH` 下的pkg目录放置的是编译期间的中间文件。
