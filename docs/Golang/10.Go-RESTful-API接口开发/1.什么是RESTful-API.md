...
这绝对是不可以被忽略的问题，如果API写得乱七八糟，很有可能会失去负责前端开发的同事的信任。

将API写得“高大上”，也是一名开发者工匠精神的一种体现。
...
|GET|http://host/resource/1|返回D为1的资源|
|POST|http://host/resource|通过POST的表单内容创建一个新的资源|
|PUT|http://host/resource/1|修改D为1的已存在资源|
|DELETE|http://host/resource/1|别除D为1的资源|
