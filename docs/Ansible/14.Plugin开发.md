...
将您的模块代码复制到适当的`plugins/`目录中。
以下代码块中使用的 copy 命令只是一个示例，让您了解要做什么——实际上，您应该选择适当的类别子目录来放置您的插件，因为它不一定适合 lookup 类别。
一旦您添加了 Python 文件，执行 git add 命令使 Git 知道新文件，然后用有意义的 commit 消息提交它。这里显示了一些示例命令：
